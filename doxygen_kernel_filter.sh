...
#   # Fix struct member docs: Convert @param / \param inside struct comments to \var
# #   s/(\/\*\*?[\s\S]*?struct[\s\S]*?\*\/)/
# #      my $c = $1;
# #      $c =~ s\/\\param\s+([a-zA-Z0-9_]+):?\/\\var $1 \/g;
# #      $c =~ s\/\@([a-zA-Z0-9_]+):?\/\\var $1 \/g;
# #      $c;
# #   /ge;

#    # Convert @param or \param inside struct blocks to clean bullet points instead of \var
#    s/(\/\*\*?\s*struct[\s\S]*?\*\/)/
#       my $c = $1;
#       $c =~ s\/\\param\s+([a-zA-Z0-9_]+):?\s*\/\\param $1 \/g;
#       $c =~ s\/\@([a-zA-Z0-9_]+):?\s*\/ - **$1**: \/g;
#       $c;
#    /ge;

#   # Transform remaining kernel-doc @param_name tags to \param param_name within block comments
#   s/(\/\*[\s\S]*?\*\/)/
#      my $c = $1;
#      $c =~ s\/\@([a-zA-Z0-9_]+)\/\\param $1\/g;
#      $c;
#   /ge;

#   # SCHED.H & TASK FLAG MACRO EXPANSIONS
#   s/TASK_PFA_TEST\s*\(\s*([A-Za-z0-9_]+)\s*,\s*([A-Za-z0-9_]+)\s*\)/bool task_$2(struct task_struct *p);/g;
#   s/TASK_PFA_SET\s*\(\s*([A-Za-z0-9_]+)\s*,\s*([A-Za-z0-9_]+)\s*\)/void task_set_$2(struct task_struct *p);/g;
#   s/TASK_PFA_CLEAR\s*\(\s*([A-Za-z0-9_]+)\s*,\s*([A-Za-z0-9_]+)\s*\)/void task_clear_$2(struct task_struct *p);/g;
#   s/TASK_PFA_FAILSAFE\s*\([^)]*\)/;/g;
